...
async function http(path, { method = "GET", body, headers } = {}) {
    const url = `${ROOT}${ensureLeadingSlash(path)}`;
...
async function httpAbs(url, { method = "GET", body, headers } = {}) {
    const token = getToken();
    const isForm = typeof FormData !== "undefined" && body instanceof FormData;
...
export function apiGetAvailability(restaurantId, dateISO) {
    ...
    return http(`/restaurants/${rid}/dashboard/${qs({ date: dateISO })}`, { method: "GET" });
}
...
export function apiGetRestaurantReservations(restaurantId, { status } = {}) {
    ...
    return http(`/restaurants/${rid}/reservations/${qs({ status })}`, { method: "GET" });
}
...
export function apiListRestaurants(params = {}) {
    return http(`/restaurants/${qs(params)}`, { method: "GET" });
}
...
export function apiModerateReservation(reservationId, nextStatus) {
    return http(`/reservations/${mustId("apiModerateReservation.reservationId", reservationId)}/moderate/`, {
        method: "POST",
...
export function apiCancelReservation(reservationId) {
    return http(`/reservations/${mustId("apiCancelReservation.reservationId", reservationId)}/cancel/`, { method: "POST" });
}

export function apiDeleteReservation(reservationId) {
    return http(`/reservations/${mustId("apiDeleteReservation.reservationId", reservationId)}/`, { method: "DELETE" });
}
...
export function apiListEvents(restaurantId, params = {}) {
    ...
    const q = sanitizeQuery({ ...params, restaurant: rid });
    return http(`/evenements/${qs(q)}`, { method: "GET" });
    // Alternative action dédiée :
...
export function apiInvite(eventId, { email, phone }) {
    return http(`/evenements/${mustId("apiInvite.eventId", eventId)}/invite/`, { method: "POST", body: { email, phone } });
}
export const apiSendInvite = apiInvite;

export function apiInviteBulk(eventId, emails = []) {
    return http(`/evenements/${mustId("apiInviteBulk.eventId", eventId)}/invite_bulk/`, { method: "POST", body: { emails } });
}
...
export async function apiSearchClients(search = "", limit = 50) {
    // essaie plusieurs chemins possibles selon ton app comptes
    const urls = [
        `${ACCOUNTS_ROOT}/users/${qs({ role: "CLIENT", search, page_size: limit })}`,
        `${ACCOUNTS_ROOT}/clients/${qs({ search, page_size: limit })}`,
        `${ACCOUNTS_ROOT}/users/search/${qs({ q: search, role: "CLIENT", page_size: limit })}`,
    ];

    for (const u of urls) {
        try {
            const raw = await httpAbs(u, { method: "GET" });
            const arr = Array.isArray(raw) ? raw : (Array.isArray(raw?.results) ? raw.results : []);
            if (!Array.isArray(arr)) continue;

            const users = arr.map((x) => ({
            ...
            return users; // endpoint OK mais vide
        } catch (e) {
            if (DEBUG) console.warn("[apiSearchClients] fallback failed for", u, e?.status || e);
        }
    }
    return [];
}
...
// ---------- Exports utilitaires ----------
export const __debug = { API, ROOT, ACCOUNTS_ROOT, getToken, toId, mustId, sanitizeQuery };
